...
while read proj; do
    ...
    if [[ "$URL" != "null" && "$URL" != "" ]]; then
        ...
        if [ $SNYK_EXIT_CODE -ge 2 ]; then
            echo "❌ ERROR [Snyk]: Falló el escaneo del contenedor $NAME. Exit code: $SNYK_EXIT_CODE."
        ...
        # FIX: Reducir payload a campos esenciales para evitar límite de n8n (~16MB)
        if ! jq --arg image "$NAME" --arg ts "$TIMESTAMP" --arg email "$USER_EMAIL" --arg group "$GROUP_ID" \
           '{
             scannedImage: $image,
             scan_timestamp: $ts,
             user_email: $email,
           ...
           "/app/docker-scan-$DOCKER_COUNT-temp.json" > "/app/docker-scan-$DOCKER_COUNT.json" 2>/tmp/jq-err-$DOCKER_COUNT.log; then
        ...
        JSON_SIZE=$(wc -c < "/app/docker-scan-$DOCKER_COUNT.json")
        echo "→ Tamaño del payload optimizado: ${JSON_SIZE} bytes"

        if ! curl -s -f -X POST "$WEBHOOK_URL/snyk-container-scan/$TICKET_ID" \
             -H "Content-Type: application/json" \
             --data-binary @/app/docker-scan-$DOCKER_COUNT.json > /tmp/curl-out-$DOCKER_COUNT.log; then
